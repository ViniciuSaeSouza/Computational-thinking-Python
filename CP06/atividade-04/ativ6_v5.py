...
"""
Procecimento: Tenta iniciar uma conexão com o banco de dados Oracle, caso tenha sucesso, define dois cursores como `inst_cadastro` e `inst_consulta` para realizar consultas e atualizações SQL;
Define a flag `conexao` como False se a conexão for mal-sucedida, define como True se obter a conexão. A flag conexão é usada para iniciar e manter o loop do menu inicial enquanto a conexão for True
"""
try:
...
Retorno: None
"""
def exibir_e_gerar_planilha(dados: pd.DataFrame, nome_arquivo_default: str = "planilha") -> None:
    if dados.empty:
        print("\n*** NÃO HÁ DADOS CORRESPONDENTES À SUA PESQUISA ***\n")
    else:
        print("\n", dados)
        planilha = input("\nGerar Planilha? [S]im ou [N]ão: ").lower()
        if planilha == "s":
...
Retorno: none
"""
def number_igual(nome_coluna:str) -> None:
    os.system('cls')
    
    texto = "idade" if nome_coluna == "idade" else "data de nascimento"    
    
    print(f"== Filtragem: {texto} do PET (igual) ==")
            
    while True:
        if nome_coluna == "data_nasc":
            print(f"\nBuscar Pet's com {texto} igual a : ")
            dia = int(input("\nDia (xx): "))
            mes = int(input("\nMês (xx): "))
            ano = int(input("\nAno (xxxxx): "))
            data = datetime(ano, mes, dia).strftime("%d/%m/%Y")
            valor = f"TO_DATE('{data}', 'DD/MM/YYYY')"
        else:
            valor = int(input(f"\nBuscar Pet's com {texto} igual a: "))
        if not valor:
    ...
    sql = f"SELECT * FROM petshop WHERE {nome_coluna} = {valor}"
    ...
    if dados:
        ...
        print(dados_df.get(["Tipo", "Nome", "Idade", "Data de Nascimento"]))
    else:
        print(f"Não conseguimos encontrar nenhum pet com {texto} igual a {valor if nome_coluna == "idade" else data}.\n")
        
        


"""
Procedimento: Recebe o nome da coluna a ser filtrada e solicita os valores correspondentes para realizar a consulta SQL MAIOR QUE, imprime na tela o `DataFrame` caso encontre algum resultado.
Parâmetro 1: nome_coluna -> Recebe qual o nome da coluna e os valores respectivos a serem solicitados para realizar a consulta.
Retorno: None
"""

def maior_que(nome_coluna:str) -> None:
    os.system('cls')
    texto = "idade" if nome_coluna == "idade" else "data de nascimento"    
    print(f"== Filtragem: {texto} do PET (maior que) ==")
            
    while True:
        if nome_coluna == "data_nasc":
            print(f"\nBuscar Pet's com {texto} maior que: ")
            dia = int(input("\nDia (xx): "))
            mes = int(input("\nMês (xx): "))
            ano = int(input("\nAno (xxxxx): "))
            data = datetime(ano, mes, dia).strftime("%d/%m/%Y")
    ...
    if dados:
        ...
        print(f"Não conseguimos encontrar nenhum pet com {texto} maior que {valor if nome_coluna == "idade" else data}.\n")



"""
Procedimento: Recebe o nome da coluna a ser filtrada e solicita os valores correspondentes para realizar a consulta SQL MENOR QUE, imprime na tela o `DataFrame` caso encontre algum resultado.
Parâmetro 1: nome_coluna -> Recebe qual o nome da coluna e os valores respectivos a serem solicitados para realizar a consulta.
Retorno: None
"""
def menor_que(nome_coluna) -> None:
    os.system('cls')
    texto = "idade" if nome_coluna == "idade" else "data de nascimento"  
    print(f"== Filtragem: {texto} do PET (menor que) ==")
            
    while True:
        if nome_coluna == "data_nasc":
            print(f"\nBuscar Pet's com {texto} menor que: ")
            dia = int(input("\nDia (xx): "))
            mes = int(input("\nMês (xx): "))
            ano = int(input("\nAno (xxxxx): "))
            data = datetime(ano, mes, dia).strftime("%d/%m/%Y")
    ...
    if dados:
        ...
        print(f"Não conseguimos encontrar nenhum pet com {texto} menor que {valor if nome_coluna == "idade" else data}.\n")


"""
Função: Solicita o `dia`, `mes` e `ano` para montar uma string de data no formato DD/MM/YYYY.
Retorno: str
"""
def cria_data_str() -> str:
    dia = input("Dia: ")
    mes = input("Mês: ")
    ano = input("Ano: ")
    data = f"{dia}/{mes}/{ano}"
...
"""
Procedimento: Recebe o nome da coluna a ser filtrada e solicita os valores correspondentes para realizar a consulta SQL ENTRE, imprime na tela o `DataFrame` caso encontre algum resultado.
Parâmetro 1: nome_coluna -> Recebe qual o nome da coluna e os valores respectivos a serem solicitados para realizar a consulta.
Retorno: None
"""
def number_entre(nome_coluna:str):
    os.system('cls')
    texto = "idade" if nome_coluna == "idade" else "data de nascimento"
    print(f"== Filtragem: {texto} do PET (entre) ==")
    while True:
        print(f"Buscar Pet's com {texto} entre - ")
        if nome_coluna == "data_nasc":
            try:
                data = cria_data_str()
                valor1 = f"TO_DATE('{data}', 'DD/MM/YYYY')"
                
                print("\nSegunda data - ")
                
                data2 = cria_data_str()
                valor2 = f"TO_DATE('{data2}', 'DD/MM/YYYY')"
                ...
                print("ERRO! ", e)
        elif nome_coluna == "idade":
            try:
                valor1 = int(input(f"{texto.capitalize()} 1: "))
                valor2 = int(input(f"{texto.capitalize()} 2: "))
                if not valor1 or not valor2:
                    print("ERRO! Campo não pode estar vazio.")
                else:
                    break
    ...
    sql = f"SELECT * FROM petshop WHERE {nome_coluna} BETWEEN {valor1} AND {valor2}"
    try:
        inst_consulta.execute(sql)
        dados = inst_consulta.fetchall()
        os.system('cls')
        if dados:
            ...
            print(f"Não conseguimos encontrar nenhum pet com {texto} entre {valor1 if nome_coluna == "idade" else data} a {valor2 if nome_coluna == "idade" else data2}.\n")
    except Exception as e:
        print("ERRO!", e)
...
Retorno: None
"""
def filtra_number(op: int, nome_coluna:str):
    match op:
        case 1:
            number_igual(nome_coluna)
        case 2:
            maior_que(nome_coluna)
        case 3:
            menor_que(nome_coluna)
        case 4:
            number_entre(nome_coluna)
...
def comeca_por_filtro(start: str) -> None:
    ...
    dados_df = pd.DataFrame.from_records(data, columns=['Id', 'Tipo', 'Nome', 'Idade', 'Data Nascimento', 'Data Cadastro'], index="Id")
...
7 - Finalizar
            ...
            if colunas_exibir and not dados_df.empty: # Se tiver colunas nas lista e `DataFrame` não está vazio
                ...
                print("\n=== Registros Filtrados ===\n")
                exibir_e_gerar_planilha(dados_filtrados)
